...
async function ensureSymlink() {
  const linkRel = path.join('module-local-dev', 'companion-module-christie-dhd800');
  ...
  try {
    ...
    if (currentTarget !== linkTarget) {
      console.log(`Symlink points to "${currentTarget}" – recreating → "${linkTarget}"`);
...
function runSync(cmd, args = []) {
  ...
  if (res.status !== 0) throw new Error(`${cmd} ${args.join(' ')} exited code ${res.status}`);
}
...
function runDev() {
  return new Promise((resolve, reject) => {
    ...
    const watch = (data) => {
      ...
      if (/Error:/i.test(text) || /ERR_/i.test(text) || /Access to this API has been restricted/i.test(text)) {
        success = false;
      }
    };
